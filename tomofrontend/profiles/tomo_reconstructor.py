...
def find_axis_hor_shift(im0, im1, xmin, xmax):
    ...
    for i in range(xmin, xmax):
        tmp_im0 = roll_zeropad(im0, shift=i, axis=1)
        tmp_im1 = roll_zeropad(im1, shift=-i, axis=1)
...
def roll_zeropad(a, shift, axis=None):
    ...
    if axis is None:
        ...
        n = a.shape[axis]
        reshape = False
    if numpy.abs(shift) > n:
        res = numpy.zeros_like(a)
    elif shift < 0:
        shift += n
        zeros = numpy.zeros_like(a.take(numpy.arange(n - shift), axis))
        res = numpy.concatenate((a.take(numpy.arange(n - shift, n), axis), zeros), axis)
    else:
        zeros = numpy.zeros_like(a.take(numpy.arange(n - shift, n), axis))
        res = numpy.concatenate((zeros, a.take(numpy.arange(n - shift), axis)), axis)
    if reshape:
